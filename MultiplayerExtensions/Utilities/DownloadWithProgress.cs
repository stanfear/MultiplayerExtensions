...
namespace MultiplayerExtensions.Utilities
{
    ...
    public class DownloadWithProgress : IDisposable
    {
        ...
        private DownloadWithProgress(HttpClient client, Uri downloadUri, int reportRate = 50)
        {
        ...
        public DownloadWithProgress(HttpClient client, Uri downloadUri, string targetFilePath, int reportRate = 50)
        ...
        public DownloadWithProgress(HttpClient client, string downloadUrl, string targetFilePath, int reportRate = 50)
        ...
        public virtual async Task StartDownload(Stream targetStream, IProgress<DownloadProgress>? progress, CancellationToken cancellationToken)
        {
            ...
            using (HttpResponseMessage response = await WebClient.GetAsync(_downloadUri, token).ConfigureAwait(false))
        ...
        /// <exception cref="OperationCanceledException"></exception>
        /// <returns></returns>
        private async Task DownloadFileFromHttpResponseMessage(HttpResponseMessage response, IProgress<DownloadProgress>? progress, CancellationToken cancellationToken)
        {
            response.EnsureSuccessStatusCode();
            HttpContent content = response?.Content ?? throw new InvalidOperationException("Content is null.");
